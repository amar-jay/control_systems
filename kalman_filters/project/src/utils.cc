...
double RMSE(const std::vector<double> &dataset) {
  ...
  double accum =
      std::accumulate(dataset.begin(), dataset.end(), 0.0,
                      [](double sum, double d) { return sum + (d * d); });

  return std::sqrt(accum / dataset.size());
}
...
std::vector<Eigen::Vector2d> generateEllipse(double x, double y,
                                             ...
                                             double sigma_xy, int num_points) {
  Eigen::Matrix2d pos_cov;
  pos_cov << sigma_xx, sigma_xy, sigma_xy, sigma_yy;

  Eigen::JacobiSVD<Eigen::MatrixXd> svd(pos_cov, Eigen::ComputeThinU |
                                                     Eigen::ComputeThinV);
  Eigen::Matrix2d D =
      svd.matrixU() *
  ...
  for (int i = 0; i < A.cols(); ++i) {
    shape_body.push_back(Eigen::Vector2d(A(0, i), A(1, i)));
  }
  std::vector<Eigen::Vector2d> shape_world =
      offsetPoints(shape_body, Eigen::Vector2d(x, y));
...
std::vector<Eigen::Vector2d> generateCircle(double x, double y, double radius,
  ...
  for (int i = 0; i < theta.size(); ++i) {
    circle.push_back(Eigen::Vector2d(x + radius * cos(theta(i)),
